...
NAME=disk2/Video/finetune/pr_llm/finetune-video-llama3.2-3b-fzy-added-acqa
...
CUDA_VISIBLE_DEVICES='3,4' python -m torch.distributed.run \
    --nproc_per_node 2 --master_port 25034 \
    train_video1.py \
    --model_name_or_path ./model/LLM/Llama-3.2-3B-Instruct \
    --version llama3 \
    --data_path ./dataset/Video/added/processed/ActivityNetQA/ActivityNetQA-opencv-pass.json \
    --image_folder ./dataset/Video/added/processed/ActivityNetQA \
