...
def build_3d_projector(config, **kwargs):
    projector_type = getattr(config, 'mm_3d_projector_type', 'linear')
    if projector_type == 'linear':
        return nn.Linear(config.mm_3d_hidden_size, config.hidden_size)
    ...
    if mlp_gelu_match:
        ...
        return build_mlp(
            mlp_depth=mlp_depth,
            input_dim=config.mm_3d_hidden_size,
            output_dim=config.hidden_size
        )
