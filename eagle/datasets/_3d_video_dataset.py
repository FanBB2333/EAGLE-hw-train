...
from eagle._3d_video_utils import VideoProcessor, merge_video_dict
...
class DataArguments:
    ...
    use_3d_coords: bool = field(default=True)
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        self.video_processor = VideoProcessor(
            ...
            voxel_size=getattr(data_args, "voxel_size", 0.1),
            min_xyz_range=getattr(data_args, "min_xyz_range", [-15, -15, -5]),
            max_xyz_range=getattr(data_args, "max_xyz_range", [15, 15, 5]),
            frame_sampling_strategy=getattr(data_args, "frame_sampling_strategy", "mc8"),
        )
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            print(f"Failed to get item {i} with error: {e}; trying to find nearby valid items instead.")
            if i > 0:
                for j in range(i-1, -1, -1):
                    try:
                        return self._getitem(j)
                    except Exception as e2:
                        print(f"Failed to get item {j} with error: {e2}")
            if i < len(self.list_data_dict) - 1:
                for j in range(i+1, len(self.list_data_dict)):
                    try:
                        return self._getitem(j)
                    except Exception as e2:
                        print(f"Failed to get item {j} with error: {e2}")
            raise e  # If all attempts fail, raise the original exception
    ...
    def _getitem(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            if 'box_input' in sources[0]:
                if isinstance(sources[0]['box_input'][0], list):
                    box_input = []
                    for box in sources[0]['box_input']:
                        box_input.append(box[:3])
                else:
                    box_input = [sources[0]["box_input"][:3]]
            else:
                box_input = None
            ...
            if "qwen2vl" in str(type(processor)).lower():
                ...
                if current_version >= target_version:
                    ...
                    processor = self.data_args.image_processor.image_processor
                if image_file.endswith(".mp4"):
                    image_list = self._get_video_frames_(
                    ...
                    image_list = video_dict.pop("images")
                if current_version >= target_version:
                    image_full = processor(videos=image_list, return_tensors='pt')
                else:
                    image_full = processor(images=None, videos=image_list, return_tensors='pt')
                # print(image_full)
        ...
        if 'image' in self.list_data_dict[i]:
            ...
            if self.data_args.use_3d_coords:
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        #     if not all(x is not None and x.shape == images[0].shape for x in images):
        #         # only stack if all instances have the same shape
