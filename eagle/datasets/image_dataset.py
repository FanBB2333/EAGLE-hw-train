...
def preprocess_qwen_2(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version == "mpt":
        ...
        assert False, f"Unsupported version: {conversation_lib.default_conversation.version}"
    # add end signal and concatenate together
    conversations = []
    for source in sources:
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            if self.data_args.image_aspect_ratio == 'pad':
                ...
                # fix ValueError: height:21 and width:170 must be larger than factor:28 
                if "qwen2vl" in str(type(processor)).lower():
                    # image = processor.preprocess(image, return_tensors='pt')
                    image = self.qwen2vlpreprocess(image)
                elif "qwen25vl" in str(type(processor)).lower():
                    # image = processor.preprocess(image, return_tensors='pt')
                    image = self.qwen25vlpreprocess(image)
                else:
                    image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
            else:
                # print(f"Processor type: {str(type(processor))}")
                if "qwen2vl" in str(type(processor)).lower():
                    # image = processor.preprocess(image, return_tensors='pt')
                    image = self.qwen2vlpreprocess(image)
                elif "qwen25vl" in str(type(processor)).lower():
        ...
        if 'image' in self.list_data_dict[i]:
            # if "qwen2vl" in str(type(processor)).lower() or "qwen25vl" in str(type(processor)).lower():
            if any(x in str(type(processor)).lower() for x in ["qwen2vl", "qwen25vl"]):
                data_dict['image'] = image['pixel_values']
                data_dict['image_grid_thw'] = image['image_grid_thw'] if 'image_grid_thw' in image else None
                data_dict['image_orig'] = image_orig
                data_dict['sources'] = sources
                # apply chat template according to the data_args's version
    ...
    def qwen2vlpreprocess(self, image):
        processor = self.data_args.image_processor
        if min(image.size[0], image.size[1]) <= 28:
            # resize image to 28 pixels
            image = image.resize((64, 64), Image.BICUBIC)
        image_p = processor.preprocess(image, return_tensors='pt')
        # print(image_p['pixel_values'].shape)
        while image_p['pixel_values'].shape[0] > 20000:
            # resize image to 20000 tokens, resize to 0.5 pixels
        ...
        if min(image.size[0], image.size[1]) <= 28:
            ...
            image_p = processor.preprocess(image, return_tensors='pt')
        return {
            'pixel_values': image_p['pixel_values'],
            'image_grid_thw': image_p['image_grid_thw'] if 'image_grid_thw' in image_p else None
        }

    def qwen25vlpreprocess(self, image):
        processor = self.data_args.image_processor
        if min(image.size[0], image.size[1]) <= 28:
            # resize image to 28 pixels
            image = image.resize((64, 64), Image.BICUBIC)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        # if 'image_orig' in instances[0] and len(instances) > 1:
        #     return self.__call__qwen2vl__(instances)
        ...
        if 'image_grid_thw' in instances[0]:
            ...
            if all(x is not None and x.shape == image_grid_thws[0].shape for x in image_grid_thws):
                batch['image_grid_thw'] = torch.stack(image_grid_thws)
            else:
                batch['image_grid_thw'] = image_grid_thws[0]

        return batch
    
    def __call__qwen2vl__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        # text = [instance['sources'] for instance in instances]
        # concat the input_ids and labels to batch
        max_length = self.data_args.tokenizer.model_max_length

        # 使用 tokenizer 对 input_ids 和 labels 进行填充和裁剪
        input_ids = [instance['input_ids'] for instance in instances]
        labels = [instance['labels'] for instance in instances]
        input_ids = self.data_args.tokenizer.pad(
            {"input_ids": input_ids},
            padding=True,   # 填充
        ...
        labels = self.data_args.tokenizer.pad(
            {"input_ids": labels},  # 使用相同的 tokenizer 对 labels 进行处理
            padding=True,   # 填充
            max_length=max_length,
            return_tensors="pt"  # 返回 PyTorch 张量
        )
        image_orig = [instance['image_orig'] for instance in instances]
        # batch = self.data_args.processor.preprocess(
        #     text=text, 
        #     images=image_orig,
        #     padding=True,
        #     return_tensors='pt')
        # batch.pop('image_orig', None)
        batch_image = self.data_args.image_processor.preprocess(
            images=image_orig,
            return_tensors='pt')
        batch = {
            ...
            'images': batch_image['pixel_values'],
            'image_grid_thw': batch_image['image_grid_thw'] if 'image_grid_thw' in batch_image else None,
        }
