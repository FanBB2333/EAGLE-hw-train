...
class EagleTrainer(Trainer):
    ...
    #             print(f"OOM error occurred, skipping batch.")
    #             # 清理缓存并跳过当前 batch
    #             # torch.cuda.empty_cache()
    #             # return 0 loss
    #             return torch.tensor(0.0, device=model.device, dtype=inputs['input_ids'].dtype)
    #         else:
    #             # 如果是其他类型的错误，重新抛出
    #             raise e
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if self.args.mm_projector_lr is not None:
                ...
                vision_tower_parameters = [name for name, _ in opt_model.named_parameters() if "vision_tower" in name]
                projector_parameters = [name for name, _ in opt_model.named_parameters() if "mm_projector" in name]
                vision_tower_projector_parameters = [name for name, _ in opt_model.named_parameters() if "vision_tower" in name or "mm_projector" in name]
                optimizer_grouped_parameters = [
                    {
                        "params": [
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, 'tune_mm_mlp_adapter', False):
            ...
            try:
                ...
                super(EagleTrainer, self)._save_checkpoint(model, trial)
