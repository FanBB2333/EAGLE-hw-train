...
class VideoProcessor:
    def __init__(
        self, 
        video_folder="/home1/hxl/Huawei/qbs_eagle/dataset/3D", 
        annotation_dir="/home1/hxl/Huawei/qbs_eagle/dataset/3D/ScanNet/Video-3D-LLM",
        voxel_size=None,
        min_xyz_range=None,
        max_xyz_range=None,
        ...
        self.video_folder = os.path.join(video_folder, "ScanNet/Video-3D-LLM/posed_images")
        ...
        filename = os.path.join(annotation_dir, f"scannet_gt_box.json")
        ...
        if 'mc' in self.frame_sampling_strategy:
            sampling_file = os.path.join(annotation_dir,"scannet_select_frames_8.json")
            ...
            with open(os.path.join(annotation_dir, 'pcd_discrete_0.1.pkl'), 'rb') as f:
    ...
    def sample_frame_files(
        ...
        video_folder = os.path.join(self.video_folder, scene_id)
        frame_files = [f for f in os.listdir(video_folder) if f.endswith(".jpg")]
        frame_files.sort()  # Ensure the frames are sorted if they are
        frame_files = [os.path.join(self.video_folder, scene_id, f) for f in frame_files]
    ...
    def calculate_world_coords(
        ...
        axis_align_matrix = torch.from_numpy(np.loadtxt(os.path.join(self.video_folder, scene_id, "axis_align_matrix.txt")))
        depth_intrinsic = torch.from_numpy(np.loadtxt(os.path.join(self.video_folder, scene_id, "depth_intrinsic.txt")))
    ...
    def process_3d_video(
        ...
        video_dict = self.preprocess(
            video_id,
            image_processor.config.image_size,
            force_sample,
            frames_upbound,
            strategy,
        )
        video_dict["images"] = image_processor(video_dict["images"])["pixel_values"]
        return video_dict
        ...
        if k in ["world_coords", 'images', 'objects']:
            ...
            for video_dict in video_dict_list:
                if video_dict[k] is not None:
                    new_video_dict['box_input'] += video_dict[k]
