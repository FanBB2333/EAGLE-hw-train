...
from eagle.datasets._3d_dataset import make_supervised_data_module, smart_tokenizer_and_embedding_resize, DataArguments
...
def train(attn_implementation=None):
    ...
    if model_args.version != 'plain':
        ...
        for name, _ in model.get_model().vision_tower.named_parameters():
            if 'vision_tower.blocks.blocks.' in name:
                # 提取层数
                layer_num = int(name.split('vision_tower.blocks.blocks.')[-1].split('.')[0])  # 根据你的命名规则提取层数
                max_layer_num = max(max_layer_num, layer_num)
        print(max_layer_num)

        # en_pr_llm
        # for name, param in model.get_model().vision_tower.named_parameters():
        #     if 'vision_tower.blocks.blocks.'  + str(max_layer_num) in name:
        #         param.requires_grad = False
        ...
        #     for name, param in model.get_model().vision_tower.named_parameters():
        #         if 'vision_tower.blocks.blocks.' + str(layer_num) + '.' in name:
        #             param.requires_grad = False
