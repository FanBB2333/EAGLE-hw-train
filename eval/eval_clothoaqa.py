...
from eval.dataset.lmms_eval_dataset import LMMsEvalDataset, clotho_aqa_tranform
...
def evaluate(args: Union[argparse.Namespace, None] = None) -> None:
    ...
    if 'video' in args.model_path.lower() or '3d' in args.model_path.lower():
        ...
        modality = 'audio'
    test_dataset = LMMsEvalDataset("/home1/hxl/disk/EAGLE/qbs/Eagle_LanguageBind/dataset/Audio/clotho_aqa/audio_files", "clotho_aqa_test", "clotho_aqa_test_filtered", transform=clotho_aqa_tranform)
    ...
    for i, data in enumerate(test_dataloader):
        ...
        #     cont = ""
        #     text_outputs = [""]
        response = text_outputs[0].strip().lower()
        outputs.append({
            'audio_id': audio_id,
            'question': question,
            'response': response,
