...
def parse_eval_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--exact_match", 
        action="store_true",
        help="Calculate exact match metric"
    )
    parser.add_argument(
        "--tasks",
        default=None,
...
def evaluate(args: Union[argparse.Namespace, None] = None) -> None:
    ...
    test_dataset = ConversationDataset(
        data_path="/home1/hxl/disk2/Backup/EAGLE/qbs/Eagle_LanguageBind/dataset/3D",
    ...
    for i, data in enumerate(test_dataloader):
        image_id, image_file, question, gt = data[0]
 
        image_tensor = image_processor(image_file, return_tensors='pt')['pixel_values'].unsqueeze(0)
    ...
    if args.exact_match:
        if isinstance(gts[0], list):
            acc = sum([1 if pred.strip().lower() in [choice.strip().lower() for choice in gt] else 0 for pred, gt in zip(preds, gts)]) / len(gts)
        else:
            acc = sum([1 if pred.strip().lower() == gt.strip().lower() else 0 for pred, gt in zip(preds, gts)]) / len(gts)
        eval_logger.info(f"Exact Match Acc: {acc:.2f}")
