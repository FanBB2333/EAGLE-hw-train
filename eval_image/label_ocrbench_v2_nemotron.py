...
def check_vllm_server_status(base_url: str = "http://localhost:58008"):
    """检查VLLM服务器状态"""
    import requests
    try:
        # 检查健康状态
...
def label_ocrv2_api(model_name_or_path: str = "Qwen2.5-VL-7B-Instruct", json_data_file: str = "OCRBench_v2.json", output_dir: str = None):
    ...
    # 首先检查VLLM服务器状态
    base_url = "http://localhost:58008/v1"
    server_base = "http://localhost:58008"
    ...
    try:
        ...
        print("Please make sure VLLM server is running on port 58008")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--method", type=str, default="api", choices=["vllm", "transformers", "api"],
    ...
    if args.method == "api":
        ...
        print("Note: Make sure VLLM server is running on port 58008")
        print("Start server with: ./start_vllm_server.sh")
        ...
        try:
            results = label_ocrv2_api(
                ...
                model_name_or_path="Llama-3.1-Nemotron-Nano-VL-8B-V1",
